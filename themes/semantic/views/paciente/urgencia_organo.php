...
Urgencias Nacionales - Organo(s) </h1>
</div>
<hr class="style-two ">

<?php 

              $organos = Yii::app()->db->createCommand()
            ->select('p.id, p.nombre, p.apellido, org.nombreOrgano, nu.grado_urgencia ,nu.fecha')
            ->from('paciente p, necesidad_organo nu, organo org ')
            ->where('p.id=nu.id_paciente AND nu.id_organo=org.idOrgano ORDER BY nu.grado_urgencia="Alto" 
...
<?php $i=1; foreach ($organos as $pa) { ?>
  ...
  <tbody>
    <tr>
      ...
      <td><?php echo $pa['nombreOrgano'] ?></td>
